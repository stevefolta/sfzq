...
void SFZEGParameters::clear_mod()
{
...
void SFZRegion::clear_for_relative_sf2()
{
	clear();
	pitch_keytrack = 0;
	amp_veltrack = 0.0;
...
void SFZRegion::add_for_sf2(SFZRegion* other)
{
	offset += other->offset;
	end += other->end;
...
void SFZRegion::sf2_to_sfz()
{
	// EG times need to be converted from timecents to seconds.
	ampeg.delay = timecents_to_secs(ampeg.delay);
	ampeg.attack = timecents_to_secs(ampeg.attack);
	ampeg.hold = timecents_to_secs(ampeg.hold);
	ampeg.decay = timecents_to_secs(ampeg.decay);
	if (ampeg.sustain < 0.0)
		ampeg.sustain = 0.0;
	ampeg.sustain = 100.0 * decibels_to_gain(-ampeg.sustain / 10.0);
	ampeg.release = timecents_to_secs(ampeg.release);
...
float SFZRegion::timecents_to_secs(short timecents)
{
